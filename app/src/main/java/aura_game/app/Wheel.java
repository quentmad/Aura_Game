...
public class Wheel<T>{
     ...
     * @param player
     */
    public void setActualToolForCategory(Player player){
     ...
     * @param player
     */
    public void setActualEquippedToolToFavoriteOfThisWheel(Player player){
        if(actualFavoriteToolForThisWheel !=null) {
            player.toolManager().setCurrentEquippedToolName(actualFavoriteToolForThisWheel.getName());
            player.toolManager().setCurrentEquippedTool(actualFavoriteToolForThisWheel);
            player.toolManager().haveAToolEquippedNow();
            if (!player.toolManager().getCurrentEquippedToolCategory().equals(NAME)) {//Si pas c'est la catégorie actuelle
                player.toolManager().setCurrentEquippedToolCategory(NAME);//Met à jour la catégorie de l'outil actuel du joueur
            }
            player.toolManager().updateEquippedToolInfo();//Met à jour les informations de l'outil actuellement équipé car l'arme équipé va directement changer (soit car on a modifié l'arme qui était de la catégorie actuelle, soit car on a changé de catégorie d'arme)

        }else{
            player.toolManager().setCurrentEquippedToolName("");
            player.toolManager().setCurrentEquippedTool(null);
        }
    }
    ...
    public boolean add(Tool element) {
        ...
        if (slotRestant > 0  ) {
            ...
            if(result){
                slotOccupied++;
                slotRestant--;
                System.out.println("Added " + element.getName() + " to "+ this);
        ...
        System.out.println("Can't add " + element.getName()+" to wheelMenu, there is no place anymore");
        return false;
    }
    ...
    public boolean drop(Player player){
        if(selectedElement != null){
            ...
            LootManager.getInstance().spawnTool(tt,player.getLootSpawnCenterX(LootableObjectType.valueOf(tt.name()).imageWidth()), player.posC().y()+LootableObjectType.valueOf(tt.name()).offsetY(),true, LootManager.getInstance().getJumpVec(mx, my, 1), selectedElement.durability());
    ...
    public void renderActualTool(SpriteBatch batch) {
        if(actualFavoriteToolForThisWheel !=null){
            ...
            batch.draw(actualFavoriteToolForThisWheel.textureBlackAndWhite(),spriteActualToolSlot.getX(), spriteActualToolSlot.getY());
        }
    }
    ...
    public void render(SpriteBatch batch){
        ...
        for(Tool element : content){
            batch.draw(element.textureBlackAndWhite(),startMenuX +  positionsSlot[i*2], startMenuY   + positionsSlot[i*2+1]-sizeSlot);
        i++;
        }
        if(slotOccupied > 0){
