...
public class Grid {
     ...
     * Rappel: tab[y] [x]*/
    private CaseListAbstractEntity[][] grid;//Car on peut pas faire un tab2D de liste
    ...
    private CaseListAbstractEntity[][] gridSuperposition;
    ...
    public Grid(int caseSize){
        ...
        this.grid = new CaseListAbstractEntity[sizeH][sizeW];
        this.gridSuperposition = new CaseListAbstractEntity[sizeH][sizeW];
        ...
        for (int i = 0; i < sizeH; i++) {
            for (int j = 0; j < sizeW; j++) {
                grid[i][j] = new CaseListAbstractEntity();
                gridSuperposition[i][j] = new CaseListAbstractEntity();
            }
        }
    }
     ...
     * Stocker cette Liste dans le AbstractEntity permet de comparer avec la nouvelle pour savoir si une mise à jour est nécessaire,
     ...
     */
    private List<Pair<Integer, Integer>> getNeighborsCasesFor(List<Pair<Integer, Integer>> gridIndex) {
     ...
     */
    public CaseListAbstractEntity getCollidingObjects(Rectangle zoneColission) {
        //Cases que survole zoneColission
        List<Pair<Integer, Integer>> pairsIndex = getCaseFor(zoneColission);
        CaseListAbstractEntity inSameCases= new CaseListAbstractEntity();
        CaseListAbstractEntity objInColission = new CaseListAbstractEntity();
     ...
     */
    public boolean isInZone(Rectangle zone, AbstractEntity objCol){
        return (zone.overlaps(objCol.hitbox().approximativeHitbox()));
    }
     ...
     * @return
     */
    public AbstractEntity getClosestObjectFrom(AbstractEntity objCible){
        //On récupère tous les objets présent (au moins partiellement) dans l/es cases où est présent l'objet cible
        List<Pair<Integer, Integer>> pairsIndex = getCaseFor(objCible.hitbox().approximativeHitbox());
        CaseListAbstractEntity inSameCases= new CaseListAbstractEntity();
        //On met dans une liste tous les objets présent (ou en partie au moins) dans les cases que survole zoneColission
        for (Pair<Integer, Integer> index : pairsIndex) {
            inSameCases.addAll(grid[index.getLeft()][index.getRight()]);
        ...
        //Garde le AbstractEntity dont la distance avec objCible est le plus court
        Pair<AbstractEntity, Double> closestO= closestObject(inSameCases, objCible);
        ...
        CaseListAbstractEntity neighborsObj = new CaseListAbstractEntity();
        for (Pair<Integer, Integer> neighborsCase : neighborsCases) {
            ...
            if (closestO.getRight() == 999999 || closestO.getRight() > (distanceBetween(objCible.hitbox().approximativeHitbox(), new Rectangle(neighborsCase.getRight(), neighborsCase.getLeft(), 1, 1)))) {
        ...
        Pair<AbstractEntity, Double> neighborsclosest = closestObject(neighborsObj, objCible);
     ...
     */
    public Pair<AbstractEntity, Double> closestObject(CaseListAbstractEntity potentialObj, AbstractEntity objCible){
        double closest = 999999;
        AbstractEntity closestObj = null;
        if(potentialObj.size() > 0){
            for (int i = 0; i < potentialObj.size(); i++) {
                double newdist = distanceBetween(objCible.hitbox().approximativeHitbox(), potentialObj.get(i).hitbox().approximativeHitbox());
                if(  newdist < closest){
                    closest = newdist;
     ...
     * si c'est le cas on appelle remove() avec la liste actuelle, on défini ensuite la liste (stocké dans AbstractEntity) au getCaseFrom() et on lance add().
     * @param obj
     * @return true s'il a été mis à jour, false sinon
     */
    public boolean update(AbstractEntity obj) {//PB: get et set non généralisé en cas de plusieurs grid
        List<Pair<Integer, Integer>> currentGridList = obj.hitbox().listIndexGrid(); // Liste actuelle
        List<Pair<Integer, Integer>> newGridList = getCaseFor(obj.hitbox().approximativeHitbox()); // Nouvelle liste
        ...
        if (!currentGridList.equals(newGridList)) { // Vérifie si la position a changé
            remove(obj); // Supprime l'objet de la grille/grille superposition
     ...
     * @param obj le AbstractEntity qu'on veut rajouter dans la grille, en fonction de son hitbox
     ...
     */
    public void add(AbstractEntity obj, List<Pair<Integer,Integer>> listIndex){
        if(listIndex.size() == 1){
            //System.out.print("1");
        ...
        obj.hitbox().setListIndexGrid(listIndex);// Définit la nouvelle liste
    }


    /**Retire obj à la grille (notamment en cas de mise à jour) de la manière suivante:
     * @param obj le AbstractEntity qu'on veut retirer de la grille, en fonction de son hitbox
     * Sinon, il est présent dans la grille principale
     */
    public void remove(AbstractEntity obj){
        List<Pair<Integer,Integer>> listIndex = obj.hitbox().listIndexGrid();
        if(listIndex.size() == 1){
 ...
 */
    ...
    /**
     * Classe interne pour stocker les abstractEntity d'une case de la grille, ou de la grille de superposition (au sein d'une liste).
     */
    class CaseListAbstractEntity {
        ...
        public void addAll(CaseListAbstractEntity lo) {
            if (lo != null) {
                list.addAll(lo.list);
            }
        }
